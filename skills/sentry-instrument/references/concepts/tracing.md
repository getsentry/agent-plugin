...
The root span is the **transaction**. Tracing answers *why* something is slow, and it’s
...
A trace is a waterfall of spans nested by ancestry, and the shape *is* the diagnosis:
...
root cause can be a *different* issue in the same request — a frontend exception, say,
...
- **Sampling is the main cost/signal lever** (trace volume dwarfs errors).
  ...
  function returning a per-transaction rate — use it to sample *down* noise (health
  checks) and *up* the paths you care about.
